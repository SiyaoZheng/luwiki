...
| 补充模态 | 官方数据页另列 IMU |
| 明确不含 | EMG、sEMG、ECG |
| 发布 | Hugging Face gated access，CC BY-NC 4.0；维护者标注总量约 1.6 TB |
...
- 数据页列出 32 通道 EEG、第一视角视频与 IMU 的目录结构，并要求使用者申请访问、遵守非商业许可、不得重新分发原始数据或尝试识别参与者。
- 维护者于 2026-07-05 宣布全部数据发布完成；这是维护者口径，使用者仍需自行核验文件完整性、匿名化处理和本地伦理许可。

## 能支持与不能支持的判断

- **可以支持**：在受控日常动作分类中研究第一视角视觉与 EEG 的多模态融合、跨被试与跨环境泛化，以及敏感视频/脑电数据的 gated 发布模式。
