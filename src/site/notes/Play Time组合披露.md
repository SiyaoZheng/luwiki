---
{"dg-publish":true,"permalink":"/Play Time组合披露/","title":"Play Time组合披露","tags":["组合披露","投资关系","组织网络"],"created":"2026-08-09","updated":"2026-08-09","dg-note-properties":{"status":"processed","title":"Play Time组合披露","source_count":2,"sources":["raw/2026-08-09-前雇主产业组织与资本端点官方记录.md","https://www.playtime.vc/"],"tags":["组合披露","投资关系","组织网络"],"created":"2026-08-09","updated":"2026-08-09"}}
---

# Play Time组合披露

Play Time组合披露是 [[Play Time\|Play Time]] 官网在 2026-08-09 可见的 portfolio 关系事件节点。

## 组合端点

- 官网直接列出并链接 [[Field AI\|Field AI]] 与 [[World Labs\|World Labs]]。
- 本事件只能证明官网组合归属，不等同于公开了一笔具体融资轮次。

## 证据等级与边界

- **证据**：A（投资方官网）。
- **不能推出**：不能推出投资日期、轮次、金额、出资基金、股比、董事安排，也不建立两个组合公司之间的直接边。

## 相关页面

- [[Play Time\|Play Time]]
- [[Field AI\|Field AI]]
- [[World Labs\|World Labs]]

## 证据

- 原始资料快照（本地归档）
- [Play Time](https://www.playtime.vc/)
