---
{"dg-publish":true,"permalink":"/香港中文大学 MMLab/","title":"香港中文大学 MMLab","tags":["实验室","香港中文大学","计算机视觉","机器学习"],"created":"2026-08-09","updated":"2026-08-10","dg-note-properties":{"status":"processed","title":"香港中文大学 MMLab","aliases":["CUHK MMLab","Multimedia Laboratory CUHK","港中文 MMLab"],"source_count":5,"sources":["raw/2026-08-09-中国高校机构官方记录-香港高校.md","raw/2026-08-10-公共机构人物补证-batch-018.md","https://mmlab.ie.cuhk.edu.hk/","https://mmlab.ie.cuhk.edu.hk/alumni.html","https://mmlab.ie.cuhk.edu.hk/people.html"],"tags":["实验室","香港中文大学","计算机视觉","机器学习"],"created":"2026-08-09","updated":"2026-08-10"}}
---

# 香港中文大学 MMLab

香港中文大学 MMLab 是[[香港中文大学\|香港中文大学]]的多媒体与计算机视觉研究网络节点，官方域名位于港中文体系。

## 与当前实体的关系

- [[黄青虬\|黄青虬]]的博士训练已由 MMLab 官方校友页直接确认：Qingqiu Huang 于 2016—2020 年在 MMLab 攻读博士，2020 年获博士学位，导师为 Dahua Lin。页面同时记录其本科和当时任职，相关机构关系只在人物页按固定中转路径建边。
- 该训练线索有助于解释墨奇智能的视觉和机器人算法来源，但不等于实验室孵化、投资或拥有公司。

## 边界

“MMLab 系”不是可替代逐人核验的团队标签；不同学校和时期的 MMLab 关联必须按作者、导师、项目与日期确认。黄青虬关系以官方校友页的 2016—2020 历史记录为界，只有实验室档案修订时才重新核对。

## 反向链接候选

- [[香港中文大学\|香港中文大学]]
- [[黄青虬\|黄青虬]]

## 证据

- 原始资料快照（本地归档）
- 原始资料快照（本地归档）
- [CUHK MMLab](https://mmlab.ie.cuhk.edu.hk/)
- [CUHK MMLab Alumni：Qingqiu Huang](https://mmlab.ie.cuhk.edu.hk/alumni.html)
- [CUHK MMLab People](https://mmlab.ie.cuhk.edu.hk/people.html)
