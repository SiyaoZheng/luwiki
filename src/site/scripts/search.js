    ...
    function setCorrectShortcut() {
        if (navigator.platform.toUpperCase().indexOf('MAC') >= 0) {
            document.querySelectorAll(".search-keys").forEach(x => x.innerHTML = "<code>⌘</code>&nbsp;+&nbsp;<code>K</code>");
        }
    }
    ...
    async function init() {
        ...
        const docs = await fetch('/searchIndex.json?v=' + encodeURIComponent(window.__searchConfig.buildDate)).then(r => r.json());
    ...
    async function search() {
        ...
        if (!searchQuery) {
            resultsDiv.innerHTML = `<span><i>${window.__searchConfig.searchNotStarted}</i></span>`;
            hidePreview();
        ...
        if (!results.length) {
            ...
            resultsDiv.innerHTML = `
                <div class="no-results">
                    ...
                    <p>${window.__searchConfig.searchNoResults} "${searchQuery}"</p>
                </div>
            `;
            return;
        }
